  ...
  return (
    <div ref={containerRef} style={{ height: `${industries.length * 100}vh` }}>
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden"
        ...
        {/* Section label */}
        <div className="px-8 sm:px-14 lg:px-20 xl:px-28 mb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 px-8 sm:px-14 lg:px-20 xl:px-28">
          ...
          {/* Right: full list — highlight active */}
          <div className="flex flex-col justify-center" style={{ borderLeft: `1px solid ${C.border}`, paddingLeft: "clamp(24px,4vw,56px)" }}>
        ...
        <div className="absolute bottom-8 right-8 sm:right-14 lg:right-20 xl:right-28 flex items-center gap-2"
          ...
          <motion.span className="text-[10px] font-mono tracking-[0.3em] uppercase"
            animate={{ opacity: [0.4, 1, 0.4] }} transition={{ repeat: Infinity, duration: 2 }}>
            scroll
          </motion.span>
          <motion.div className="w-px h-6 rounded-full" style={{ background: `linear-gradient(to bottom, ${C.accent}, transparent)` }}
...
export default function Home() {
  return (
    <Layout>
      ...
      {/* ══════════════════════════════════════════
          HERO — white, split, product mockup
      ══════════════════════════════════════════ */}
      <section className="flex items-center px-8 sm:px-14 lg:px-20 xl:px-28"
        style={{ backgroundColor: C.bg, minHeight: "calc(100vh - 72px)", paddingTop: "clamp(56px,9vh,90px)", paddingBottom: "clamp(56px,9vh,90px)" }}>
        <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-28 items-center">
          ...
          <div>
            <motion.p
              ...
              className="text-[10px] font-mono tracking-[0.35em] uppercase mb-8"
              style={{ color: C.muted }}
            >
            ...
            <motion.h1
              ...
              className="font-black tracking-[-0.04em] leading-[1.04] mb-6"
          ...
          >
            <motion.div
              ...
              style={{ border: `1px solid ${C.border}`, boxShadow: "0 20px 60px rgba(0,0,0,0.08), 0 4px 12px rgba(0,0,0,0.04)" }}
            >
              {/* Browser chrome */}
              <div className="flex items-center gap-2 px-4 py-3" style={{ backgroundColor: C.soft, borderBottom: `1px solid ${C.border}` }}>
                <div className="flex gap-1.5">
                  {["#fca5a5","#fde68a","#6ee7b7"].map((c) => <div key={c} className="w-3 h-3 rounded-full" style={{ backgroundColor: c }} />)}
                </div>
              ...
              <div className="flex" style={{ backgroundColor: C.bg, minHeight: 400 }}>
                {/* Sidebar */}
                <div className="w-48 shrink-0 py-5 px-3" style={{ backgroundColor: C.soft, borderRight: `1px solid ${C.border}` }}>
                  <p className="px-3 py-1.5 mb-4 text-[9px] font-mono tracking-[0.2em] uppercase" style={{ color: "#aaa" }}>AI Admin Co-Pilot</p>
                  {[{ label: "Dashboard", on: true },{ label: "Workflows", on: false },{ label: "Documents", on: false },{ label: "Compliance", on: false },{ label: "Reports", on: false }].map((it) => (
                    <div key={it.label} className="flex items-center gap-2.5 px-3 py-2 text-xs font-medium mb-0.5"
                      ...
                      <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: it.on ? C.text : C.border }} />
                      {it.label}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex-1 p-5">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ backgroundColor: C.border }}>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    {[
                      ...
                      <div key={m.label} className="p-3" style={{ backgroundColor: C.soft, border: `1px solid ${C.border}` }}>
                  ...
                  <div style={{ border: `1px solid ${C.border}` }}>
                    <div className="px-4 py-2.5 flex items-center justify-between" style={{ backgroundColor: C.soft, borderBottom: `1px solid ${C.border}` }}>
                    ...
                    {[
                      ...
                      <div key={w.name} className="px-4 py-3" style={{ borderBottom: wi < 2 ? `1px solid ${C.border}` : "none" }}>
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[11px] font-medium" style={{ color: C.text }}>{w.name}</span>
                          <span className="text-[10px] font-medium px-2 py-0.5" style={{ color: w.sc, backgroundColor: w.sc + "15" }}>{w.status}</span>
                        </div>
            ...
            <p className="text-center text-xs mt-4" style={{ color: C.muted }}>AI Admin Co-Pilot — Healthcare & Allied Health platform</p>
          </motion.div>
        </div>
      </section>

      {/* ══════════════════════════════════════════
          TICKER — monochrome, subtle
      ══════════════════════════════════════════ */}
      <div className="overflow-hidden py-4" style={{ backgroundColor: C.bg, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
        <div className="animate-marquee">
          {[...ticker, ...ticker].map((item, i) => (
            <span key={i}
              className="inline-flex items-center gap-5 px-6 text-[10px] font-mono tracking-[0.32em] uppercase whitespace-nowrap"
              style={{ color: i % 3 === 0 ? C.text : C.muted }}>
              {item}
      ...
      {/* ══════════════════════════════════════════
          MANIFESTO — black left + photo right
      ══════════════════════════════════════════ */}
      <section className="grid grid-cols-1 lg:grid-cols-2">
        <div className="flex flex-col justify-between px-8 sm:px-14 lg:px-20 xl:px-28 py-24"
          ...
          <div>
            ...
            <Reveal delay={0.16}>
              <p className="text-base leading-relaxed mb-14 max-w-sm" style={{ color: "rgba(255,255,255,0.5)" }}>
                Healthcare. Financial. Legal. Each industry has its own language, risk profile, and compliance landscape. We don't adapt generic models — we build specific ones.
              </p>
            </Reveal>
            <Reveal delay={0.26}>
              <Link href="/about">
        ...
        <div className="relative min-h-[380px] lg:min-h-0 overflow-hidden">
          ...
          <div className="absolute inset-0"
            style={{ background: "linear-gradient(135deg, rgba(0,0,0,0.15) 0%, transparent 55%)" }} />
        </div>
      </section>

      {/* ══════════════════════════════════════════
          PLATFORMS — white + clean borders
      ══════════════════════════════════════════ */}
            ...
            <Link href="/platforms">
              ...
              >
                ...
                <div className="relative overflow-hidden min-h-[280px]" style={{ order: i % 2 === 0 ? 2 : 1 }}>
                  ...
                  <div className="absolute inset-0" style={{ backgroundColor: "rgba(0,0,0,0.12)", mixBlendMode: "multiply" }} />
                </div>
              </motion.div>
            </Link>
          </motion.div>
      ...
      <section className="py-24 px-8 sm:px-14 lg:px-20 xl:px-28"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 sm:gap-0"
          ...
          {[
            ...
            <Reveal key={s.n} delay={i * 0.1}
              className="px-10 sm:px-12 first:pl-10 last:pr-0 sm:border-r"
              style={{ borderColor: "rgba(255,255,255,0.1)" } as any}
            >
              <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.25 }}>
                <span className="text-[10px] font-mono tracking-widest block mb-8" style={{ color: "rgba(255,255,255,0.3)" }}>{s.n}</span>
        ...
        <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }}
          ...
          className="h-px origin-left mt-16 mb-10"
      ...
      {/* ══════════════════════════════════════════
          CTA — white with thin border top
      ══════════════════════════════════════════ */}
      <section className="py-36 px-8 sm:px-14 lg:px-20 xl:px-28"
        style={{ backgroundColor: C.bg, borderTop: `1px solid ${C.border}` }}>
        <Reveal>
          <p className="text-[10px] font-mono tracking-[0.35em] uppercase mb-10" style={{ color: C.muted }}>
            Get started
          </p>
        </Reveal>
        <Reveal delay={0.08}>
